...
public class MutualFriend {
	public static class MyMapper extends Mapper<Object, Text, Text, Text> { 
		public void map(Object key, Text value, Context context) throws IOException, InterruptedException {
			...
			String[] valuesplit = line[j].split(" ");
			for(int i = 2; i < valuesplit.length; i++){
				int compare = valuesplit[i].compareTo(valuesplit[0]);
				if(compare > 0){
					wordKey.set(valuesplit[0] + "," +valuesplit[i]);
					context.write(wordKey, new Text(valuesplit[i]));
				}
				else if(compare < 0){
					wordKey.set(valuesplit[i] + "," +valuesplit[0]);
					context.write(wordKey, new Text(valuesplit[i]));
				}
			}
		}
